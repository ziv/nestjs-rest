import {BadRequestException, Body, Delete, Get, Header, NotFoundException, Param, Patch, Post,} from "@nestjs/common";
import {
    AttributesObject,
    CollectionResourceDocument,
    PlainDocument, ResourceObject,
...
> {
    ...
    async getRecordById(
        ...
        // example for class builder composition
        return JsonApiBuilder
            .singleDocument<Model>()
            .metadata({id})
            .links({})
            .data(
                JsonApiBuilder
                    .resource<Model>(res.data.id, this.options.resourceId)
                    .links({})
    ...
    async createRecord(
        ...
        return JsonApiBuilder.singleDocument<PlainDocument>().metadata(metaData).build();
    }
    ...
    async updateRecord(
        ...
        return JsonApiBuilder.singleDocument().metadata(metaData).as<PlainDocument>();
    }

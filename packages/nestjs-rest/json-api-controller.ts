...
export class JsonApiController {
    ...
    async multiple(@JsonQuery() query: JsonApiQuery): Promise<JsonApiCollectionDocument> {
        const result = await this.options.adapter.multiple(query);

        // Transform models to resource objects
        const resources = result.data.map(model => this.transformModelToResource(model));

        // Build JSON:API collection document
    ...
    async single(@Param("id") id: string): Promise<JsonApiSingleDocument> {
        try {
            ...
            // Transform model to resource object
            const resource = this.transformModelToResource(model, id);

            // Build JSON:API single document
    ...
    async create(@Body() body: { data: { attributes: Attributes } }): Promise<JsonApiSingleDocument> {
        const createResponse = await this.options.adapter.create(body.data.attributes);

        // Fetch the created resource to return it
        const model = await this.options.adapter.single(createResponse.meta.id);
        const resource = this.transformModelToResource(model, createResponse.meta.id);

        // Build JSON:API single document with creation metadata
        return new JsonApiDocumentBuilder()
            .data(resource)
            .metadata({
            ...
            .links({
                self: `${this.options.resourceDescriptor.baseUrl}/${createResponse.meta.id}`
            })
            .build();
    }
    ...
    async update(
        ...
        try {
            const updateResponse = await this.options.adapter.update(id, body.data.attributes);

            // Fetch the updated resource to return it
            const model = await this.options.adapter.single(id);
            const resource = this.transformModelToResource(model, id);

            // Build JSON:API single document with update metadata
            return new JsonApiDocumentBuilder()
                .data(resource)
                .metadata({
     ...
     * @internal
     */
    protected transformModelToResource(model: Attributes, explicitId?: string): ResourceObject {

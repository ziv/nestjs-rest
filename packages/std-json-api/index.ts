...
export class JsonApiBuilder<T extends AttributesObject = AttributesObject> {
    ...
    included(resources: ResourceObject[]): this {
        if ('data' in this.document) {
            (this.document as SingleResourceDocument<T> | CollectionResourceDocument<T>).included = resources;
        }
        return this;
    }
    ...
    build(): JsonApiDocument<T> {
        if ('errors' in this.document) {
            return this.document as ErrorDocument;
        }

        if ('data' in this.document) {
            if (Array.isArray((this.document as any).data)) {
                return this.document as CollectionResourceDocument<T>;
            } else {
                return this.document as SingleResourceDocument<T>;
            }
        }
...
export class ResourceBuilder<T extends AttributesObject = AttributesObject> {
    ...
    build(): ResourceObject<T> {
        return this.resource as ResourceObject<T>;
    }
}
...
export class JsonApi {
     ...
     */
    static document<T extends AttributesObject = AttributesObject>(): JsonApiBuilder<T> {
     ...
     */
    static resource<T extends AttributesObject = AttributesObject>(id: string, type: string): ResourceBuilder<T> {
     ...
     */
    static resourceIdentifier(id: string, type: string, meta?: Meta): ResourceIdentifierObject {
        return {id, type, meta};
    }
     ...
     */
    static parse<T extends AttributesObject = AttributesObject>(json: string): JsonApiDocument<T> {
    ...
    static validate(obj: any): boolean {
        ...
        if (hasData) {
            const data = obj.data;
            if (data !== null && !this.isValidResourceObject(data) && !Array.isArray(data)) {
                return false;
            }
            if (Array.isArray(data) && !data.every(item => this.isValidResourceObject(item))) {
                return false;
            }
        }

import type {Collection, Document, Filter, MatchKeysAndValues, OptionalUnlessRequiredId,} from "mongodb";
...
export default class MongodbAdapter<T extends Document & AttributesObject>
    ...
    async multiple(
        ...
        const $project = (input.fields && input.fields[this.options.id])
            ? input.fields[this.options.id].split(',').reduce((acc: { [key: string]: 1 }, cur: string) => ({
                ...acc,
                [cur]: 1
            }), {})
            : {};

        const $sort = (input.sort && Object.keys(input.sort ?? {}).length > 0) ? input.sort : {_id: 1}

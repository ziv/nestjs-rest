...
export default class MongodbAdapter<T extends Doc>
  ...
  constructor(options: Partial<MongodbAdapterOptions<T>>) {
    ...
    if (options.collections) {
      ...
      throw new Error(
        "MongodbAdapter requires either collections or collection and relatedCollections",
      );
    }
  ...
  async multiple(input: JsonApiQuery): Promise<CollectionResourceDocument> {
     ...
     */
    pipeline.push(
      (Object.keys(input.sort).length > 0)
        ? { $sort: input.sort }
        : { $sort: { _id: 1 } },
    );
    ...
    const [{ docs, total }] = await this.options.collection.aggregate(pipeline)
      .toArray();

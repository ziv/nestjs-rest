...
export type MongodbAdapterOptions<T extends Doc> = {
     ...
     * Define how pagination is handled in queries and how data is retrieved.
     */
    pipeline: (opts: MongodbAdapterOptions<any>, query: JsonApiQuery) => Document[]
};

export function normalize<T extends Doc>(options: Partial<MongodbAdapterOptions<T>>) {
    ...
    if (!options.uniqueFactory) {
        ...
        if (!key) {
            ...
            throw new Error("MongoAdapter requires idKey in ResourceDescriptor to build uniqueFactory");
        } else if (Array.isArray(key)) {
            // composite key
            options.uniqueFactory = (ids: InputId) => {
                if (ids.length !== key.length) {
                    throw new Error(`Composite key requires ${key.length} values, but got ${ids.length}`);
                }
            ...
            // mongo object id
            options.uniqueFactory = (id: InputId) => ({_id: new ObjectId(id as string)} as MatchKeysAndValues<never>);
        } else {
            // simple key
            options.uniqueFactory = (id: InputId) => ({[key]: id} as MatchKeysAndValues<never>);
        }
    }
    ...
    if (!options.pipeline) {
        // default to offset pagination pipeline
        options.pipeline = OffsetPipeline;
    }
